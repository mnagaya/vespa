...
%destructor { delete $$; } ident bool_ null_ string doc_type value number leaf comparison
%destructor { delete $$; } logical_expr expression arith_expr id_spec variable field_spec mangled_ident
...
%right UNOT
%left NON_DOT
%precedence LPAREN DOT /* TODO verify */

%%

null_
    : NULL { $$ = new NullValueNode("null"); } /* TODO remove string from AST node */
    ;

bool_
    : TRUE   { $$ = new Constant("true"); } /* TODO remove strings from AST constant nodes */
...
string
    : STRING { {
          try {
              ...
              // Rethrow as Parser-specific error to ensure proper cleanup and termination
...
doc_type
 ...
 /* TODO test case preservation when scheme etc is used in an ident context */
ident
    : IDENTIFIER { $$ = $1; }
    | SCHEME     { $$ = $1; }
    | TYPE       { $$ = $1; }
...
id_spec
    : ID %prec NON_DOT { $$ = new IdValueNode(bucket_id_factory, "id", ""); } /* TODO uhhh verify precedence hack */
...
arith_expr
    : value                         { $$ = $1; }
    | value "." IDENTIFIER "(" ")"  { $$ = new FunctionValueNode(*steal<string>($3), steal<ValueNode>($1)); } /* FIXME shift/reduce */
    | number                        { $$ = $1; } 
    | number "." IDENTIFIER "(" ")" { $$ = new FunctionValueNode(*steal<string>($3), steal<ValueNode>($1)); } /* FIXME shift/reduce */
    ...
    | arith_expr "/" arith_expr { $$ = new ArithmeticValueNode(steal<ValueNode>($1), "/", steal<ValueNode>($3)); }
    | arith_expr "%" arith_expr { $$ = new ArithmeticValueNode(steal<ValueNode>($1), "%", steal<ValueNode>($3)); }
    | "(" arith_expr ")"        { $$ = $2; $$->setParentheses(); } /* FIXME reduce/reduce conflict with logical_expr */
...
leaf
    ...
    | field_spec {
          ...
          $$ = new Compare(steal<FieldExprNode>($1)->convert_to_field_value(),
                           FunctionOperator::NE,
                           std::make_unique<NullValueNode>("null"),
                           bucket_id_factory);
      }
    ;
...
logical_expr
    ...
    | "(" logical_expr ")"          { $$ = $2; $$->setParentheses(); } /* FIXME reduce/reduce conflict with arith_expr */
    ;
